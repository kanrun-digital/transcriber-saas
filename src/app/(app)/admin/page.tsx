...
function ModelsTab() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-lg font-semibold">Chat моделі ({chatModels.length})</h3>
        <div className="rounded-lg border mt-2">
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead>Модель</TableHead>
                <TableHead>Coins/100w</TableHead>
                <TableHead className="hidden md:table-cell">Word Limit</TableHead>
                <TableHead className="hidden md:table-cell">Max Output</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {chatModels.map((m: any) => (
                <TableRow key={m.id}>
                  <TableCell className="font-mono text-xs">{m.name || m.id}</TableCell>
                  <TableCell><Badge variant="outline">{m.coins}</Badge></TableCell>
                  <TableCell className="hidden md:table-cell text-sm text-muted-foreground">{m.wordLimit?.toLocaleString() || "—"}</TableCell>
                  <TableCell className="hidden md:table-cell text-sm text-muted-foreground">{m.maxOutput?.toLocaleString() || "—"}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
      ...
      <div>
        <h3 className="text-lg font-semibold">Image моделі ({imageModels.length})</h3>
        <div className="rounded-lg border mt-2">
          <Table>
            <TableHeader>
              <TableRow>
  ...
  return (
    <div className="rounded-lg border">
      <Table>
        ...
        <TableBody>
          {agents.length === 0 ? (
            <TableRow><TableCell colSpan={5} className="text-center">Агентів не знайдено</TableCell></TableRow>
          ) : agents.filter((a: any) => a.status !== "deleted").map((agent: any) => (
            <TableRow key={agent.id}>
              <TableCell className="font-medium">{agent.name || "—"}</TableCell>
              <TableCell className="text-sm">{agent.ownerEmail || "—"}</TableCell>
              <TableCell><Badge variant={agent.status === "active" ? "default" : "secondary"}>{agent.status}</Badge></TableCell>
              <TableCell className="hidden md:table-cell text-xs font-mono truncate max-w-[150px]">{agent.straico_agent_id || "—"}</TableCell>
              <TableCell>
                <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive"
                  onClick={() => { if (confirm("Видалити агента?")) deleteMut.mutate(agent.id); }}>
    ...
    <div className="space-y-6">
      ...
      <Tabs value={tab} onValueChange={setTab}>
        <TabsList>
          ...
          <TabsTrigger value="models">AI Моделі</TabsTrigger>
