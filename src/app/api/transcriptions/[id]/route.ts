...
/**
 * GET /api/transcriptions/[id] — get transcription (excludes soft-deleted)
 * DELETE /api/transcriptions/[id] — soft delete + S3 cleanup + RAG cleanup
 */
...
export async function GET(
  ...
  try {
    ...
    const tx = await ncb.readOne<any>("transcriptions", Number(params.id));
    if (!tx || tx.deleted_at) {
...
export async function DELETE(
  ...
  try {
    ...
    // 6. Soft-delete transcription
    await ncb.update("transcriptions", txId, {
      deleted_at: now(),
      updated_at: now(),
    });

    // 7. Release storage quota
